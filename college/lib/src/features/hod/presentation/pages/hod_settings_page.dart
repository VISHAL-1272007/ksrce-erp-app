...
class _HodSettingsPageState extends State<HodSettingsPage> {
  ...
  void _loadFromDs(DataService ds) {
    ...
    _attendanceAlerts = s['attendanceAlerts'] as bool? ?? true;
    _loaded = true;
  }
        ...
        body: SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: const [
              ...
              SizedBox(width: 12),
              Text('HOD Settings', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: AppColors.textDark)),
            ]),
            const SizedBox(height: 24),
            _buildNotificationSettings(ds),
            const SizedBox(height: 24),
  ...
  Widget _buildNotificationSettings(DataService ds) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _toggle('Email Notifications', _emailNotif, (v) { setState(() => _emailNotif = v); _persist(ds); }),
        _toggle('SMS Notifications', _smsNotif, (v) { setState(() => _smsNotif = v); _persist(ds); }),
        _toggle('Push Notifications', _pushNotif, (v) { setState(() => _pushNotif = v); _persist(ds); }),
        const Divider(color: AppColors.border, height: 24),
        _toggle('Leave Request Alerts', _leaveNotif, (v) { setState(() => _leaveNotif = v); _persist(ds); }),
        _toggle('Low Attendance Alerts', _attendanceAlerts, (v) { setState(() => _attendanceAlerts = v); _persist(ds); }),
      ]),
    );
  }
  ...
  Widget _buildAccountInfo(Map<String, dynamic> fac, String fid, String dept) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _infoRow('Role', 'Head of Department'),
