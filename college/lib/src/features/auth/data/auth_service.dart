...
class AuthService {
  ...
  Future<LoginResult> login(String userId, String password, bool rememberMe) async {
    ...
    // Bypass the blocked Render API and use local mock data so the user can work on UI.
    try {
      if (normalizedUserId == 'admin@ksrce.ac.in' && password == 'Admin@1234') {
        final ds = DataService();
        await ds.forceLogin(normalizedUserId, 'admin');
      ...
      await _logLoginAttempt(prefs, normalizedUserId, false);
      return _handleFailedLogin(prefs, 'Local mock error: ${e.toString()}');
    }
  }
  ...
  /// Logs the user out: clears the JWT token and remembered user.
