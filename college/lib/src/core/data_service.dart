...
import 'services/smart_timetable_scheduler.dart';
...
class DataService extends ChangeNotifier {
  ...
  /// Current authenticated user (Student, Faculty, Admin, or HOD)
  ...
  /// 2. Apply any pending offline edits from previous sessions
  Future<void> loadAllData() async {
    ...
    try {
      ...
      // ── STEP 3: First run — seed from bundled JSON assets ──
  ...
  /// Syncs Students, Attendance, and Courses from the FastAPI backend
  /// into the local in-memory cache.
  ///
  /// - Only runs if a valid JWT is stored (i.e. user is logged in via backend).
  /// - Silently falls back to offline data if the network is unreachable.
  /// - Deduplicates records by their primary ID field so merging is safe.
  /// - Persists the updated data locally after a successful sync.
  Future<void> syncFromBackend() async {
    if (!await ApiClient.isAuthenticated()) {
      debugPrint('[DataService] syncFromBackend: no token, skipping.');
      return;
    }
    ...
    // ── 1. Students ──────────────────────────────────────────────────────
    final studentsResp = await client.get('/students');
    if (studentsResp.success && studentsResp.dataAsList.isNotEmpty) {
      final remote = studentsResp.dataAsList;
      final merged = _mergeById(
        existing: _students,
        incoming: remote,
        idKey: 'id',        // backend key
        localIdKey: 'studentId', // frontend key
      );
      if (merged.changed) {
        _students = merged.result;
        didUpdate = true;
        debugPrint('[DataService] syncFromBackend: students updated (${remote.length} records).');
      }
    } else if (studentsResp.isUnauthorized) {
      debugPrint('[DataService] syncFromBackend: 401 on /students — token cleared.');
      return;
    } else if (!studentsResp.isNetworkError) {
      debugPrint('[DataService] syncFromBackend /students error: ${studentsResp.error}');
    }

    // ── 2. Attendance ────────────────────────────────────────────────────
    final attendanceResp = await client.get('/attendance');
    if (attendanceResp.success && attendanceResp.dataAsList.isNotEmpty) {
      final remote = attendanceResp.dataAsList;
      final merged = _mergeById(
        existing: _attendance,
        incoming: remote,
        idKey: 'id',
        localIdKey: 'attendanceId',
      );
      if (merged.changed) {
        _attendance = merged.result;
        didUpdate = true;
        debugPrint('[DataService] syncFromBackend: attendance updated (${remote.length} records).');
      }
    } else if (attendanceResp.isUnauthorized) {
      debugPrint('[DataService] syncFromBackend: 401 on /attendance — token cleared.');
      return;
    } else if (!attendanceResp.isNetworkError) {
      debugPrint('[DataService] syncFromBackend /attendance error: ${attendanceResp.error}');
    }

    // ── 3. Courses ───────────────────────────────────────────────────────
    final coursesResp = await client.get('/courses');
    if (coursesResp.success && coursesResp.dataAsList.isNotEmpty) {
      final remote = coursesResp.dataAsList;
      final merged = _mergeById(
        existing: _courses,
        incoming: remote,
        idKey: 'id',
      ...
      if (merged.changed) {
        _courses = merged.result;
        didUpdate = true;
        debugPrint('[DataService] syncFromBackend: courses updated (${remote.length} records).');
      }
    } else if (!coursesResp.isNetworkError) {
      debugPrint('[DataService] syncFromBackend /courses error: ${coursesResp.error}');
    }

    // ── 4. Results ───────────────────────────────────────────────────────
    final resultsResp = await client.get('/results');
    if (resultsResp.success && resultsResp.dataAsList.isNotEmpty) {
      final remote = resultsResp.dataAsList;
      final merged = _mergeById(
        existing: _results,
        incoming: remote,
        idKey: 'id',
      ...
      if (merged.changed) {
        _results = merged.result;
        didUpdate = true;
        debugPrint('[DataService] syncFromBackend: results updated (${remote.length} records).');
      }
    } else if (!resultsResp.isNetworkError) {
      debugPrint('[DataService] syncFromBackend /results error: ${resultsResp.error}');
    }

    // ── 5. Assignments ───────────────────────────────────────────────────
    final assignmentsResp = await client.get('/assignments');
    if (assignmentsResp.success && assignmentsResp.dataAsList.isNotEmpty) {
      final remote = assignmentsResp.dataAsList;
      final merged = _mergeById(
        existing: _assignments,
        incoming: remote,
        idKey: 'id',
        localIdKey: 'assignmentId',
      );
      if (merged.changed) {
        _assignments = merged.result;
        didUpdate = true;
        debugPrint('[DataService] syncFromBackend: assignments updated (${remote.length} records).');
      }
    } else if (!assignmentsResp.isNetworkError) {
      debugPrint('[DataService] syncFromBackend /assignments error: ${assignmentsResp.error}');
    }

    // ── 6. Timetable ─────────────────────────────────────────────────────
    final timetableResp = await client.get('/timetable');
    if (timetableResp.success && timetableResp.dataAsList.isNotEmpty) {
      final remote = timetableResp.dataAsList;
      final merged = _mergeById(
        existing: _timetable,
        incoming: remote,
        idKey: 'id',
        localIdKey: 'timetableId',
      );
      if (merged.changed) {
        _timetable = merged.result;
        didUpdate = true;
        debugPrint('[DataService] syncFromBackend: timetable updated (${remote.length} records).');
      }
    } else if (!timetableResp.isNetworkError) {
      debugPrint('[DataService] syncFromBackend /timetable error: ${timetableResp.error}');
    }

    // ── 7. Notifications ─────────────────────────────────────────────────
    final notifResp = await client.get('/notifications');
    if (notifResp.success && notifResp.dataAsList.isNotEmpty) {
      final remote = notifResp.dataAsList;
      // Normalise backend snake_case → camelCase
      final normalised = remote.map((e) => {
        ...e,
        'notificationId': e['id'] ?? e['notificationId'],
        'isRead': (e['is_read'] == true || e['is_read'] == 1),
      ...
      final merged = _mergeById(
        ...
        localIdKey: 'notificationId',
      );
      if (merged.changed) {
        _notifications = merged.result;
        didUpdate = true;
        debugPrint('[DataService] syncFromBackend: notifications updated (${remote.length} records).');
      }
    } else if (!notifResp.isNetworkError) {
      debugPrint('[DataService] syncFromBackend /notifications error: ${notifResp.error}');
    }
    ...
    if (didUpdate) {
      ...
      await PersistenceService.saveAll(_buildFullMap(),
          changedKeys: ['students', 'attendance', 'courses', 'results', 'assignments', 'timetable', 'notifications']);

      debugPrint('[DataService] syncFromBackend: persisted merged data.');
    }
  }

  /// Merges [incoming] backend records into [existing] local records.
  /// Uses [idKey] from incoming and [localIdKey] from existing to match.
  /// Backend record fields take priority; unmatched local records are kept.
  _MergeResult _mergeById({
    required List<Map<String, dynamic>> existing,
    required List<Map<String, dynamic>> incoming,
    required String idKey,
    required String localIdKey,
  }) {
    if (incoming.isEmpty) return _MergeResult(result: existing, changed: false);

    // Build lookup: backendId -> existing local record
    final Map<String, Map<String, dynamic>> localById = {};
    for (final item in existing) {
      final id = item[localIdKey]?.toString() ?? item[idKey]?.toString() ?? '';
      if (id.isNotEmpty) localById[id] = item;
    }

    bool changed = false;
    for (final remoteItem in incoming) {
      final rid = remoteItem[idKey]?.toString() ?? '';
      if (rid.isEmpty) continue;
      if (!localById.containsKey(rid)) {
        // New record from backend — add it
        localById[rid] = {...remoteItem, localIdKey: rid};
        changed = true;
      } else {
        // Update only backend-owned fields (do not overwrite purely local fields)
        final merged = Map<String, dynamic>.from(localById[rid]!);
        remoteItem.forEach((k, v) {
          if (v != null && merged[k] != v) {
            merged[k] = v;
            changed = true;
          }
        });
        localById[rid] = merged;
      }
    }

    return _MergeResult(
      result: localById.values.toList(),
      changed: changed,
    );
  }
  ...
  void _hydrateFromMap(Map<String, dynamic> data) {
    ...
    // Migrate any plaintext passwords loaded from the local cache to hashed form.
    final cachedUsers = _asList(data['users']);
    _users = cachedUsers.map((u) {
      final raw = u['password'] as String? ?? '';
      final id = u['id'] as String? ?? '';
      if (raw.isNotEmpty && raw.length != 64) {
        return Map<String, dynamic>.from(u)
          ..['password'] = SecurityService.hashPassword(raw, id);
      }
      return Map<String, dynamic>.from(u);
    }).toList();
  ...
  Future<void> loadForRole(String role) async {
    // Since we are mocking the login locally for the admin to bypass the blocked Render API,
    // we must also load the local mock data instead of relying on the API!
    if (!_isLoaded) {
      await _seedFromAssets();
      _isLoaded = true;
      notifyListeners();
  ...
  Future<void> _seedFromAssets() async {
    ...
    _students = futures[0];
    // Hash plaintext passwords from users.json at seed time so loginSecure works.
    // users.json stores raw passwords for readability; they must be converted to
    // SHA-256 hashes (pepper + userId salt) before the first login attempt.
    final rawUsers = futures[1] as List<Map<String, dynamic>>;
    _users = rawUsers.map((u) {
      final raw = u['password'] as String? ?? '';
      final id = u['id'] as String? ?? '';
      // Only hash if the stored value looks like a plaintext password (not a 64-char hex hash)
      if (raw.isNotEmpty && raw.length != 64) {
        return Map<String, dynamic>.from(u)
          ..['password'] = SecurityService.hashPassword(raw, id);
      }
  ...
  /// Note: The cloud sync is now handled by FastAPI.
  ...
  /// Safely cast a dynamic value to List<Map<String, dynamic>>.
      ...
      if (user['id'] == userId) {
        ...
        if (SecurityService.verifyPassword(password, userId, storedHash)) {
          ...
          if (_currentRole != null) {
            ...
            // Non-blocking: sync fresh data from the FastAPI backend
            syncFromBackend();
          }
          notifyListeners();
          return null; // success
  ...
  Future<void> forceLogin(String userId, String role) async {
    _currentUserId = userId;
    _currentRole = role;
    notifyListeners(); // Notify immediately so permission checks work
    await loadForRole(role); // Now properly awaited — data will be ready
  }
  ...
  Map<String, dynamic> getEffectivePermissionsForUser({String? userId}) {
    ...
    if (targetUserId == 'admin@ksrce.ac.in' || targetUserId == 'ADM001' || _currentRole == 'admin') {
      return {'portalRole': 'admin', 'view': <String>['*'], 'edit': <String>['*']};
    }
...
/// Return type for [DataService._mergeById].
class _MergeResult {
  final List<Map<String, dynamic>> result;
  final bool changed;
  const _MergeResult({required this.result, required this.changed});
}
